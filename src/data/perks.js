 ...
 *
 * unlockedBy — единственная карточка в игре, которая переживает забег. Найдя
 * её однажды за 300 секунд в пустоте, игрок получает одноимённое достижение,
 * и с этого момента карточка начинает ходить в обычном пуле БУДУЩИХ забегов.
 * Взятая уже там, она открывает третью концовку (systems/endings.js).
 *
 * weight подобран так, чтобы она попадалась примерно в четверти-трети забегов:
 * реже — и «тайна» превращается в недостижимую строчку в бестиарии, чаще —
 * перестаёт быть тайной. Проверяется в bench/endings_check.mjs.
 */
export const SINGULARITY_PERK = {
  id: 'singularity_patience', icon: '◉', name: 'ТЕРПЕНИЕ ПУСТОТЫ', rarity: 'legendary', max: 1, weight: 10,
  exclusiveSource: 'singularity', unlockedBy: 'singularity_patience', tags: [],
